...
/// </summary>
public partial class ProductAttributeFormatterCustomized : IProductAttributeFormatter, IProductAttributeFormatterCustomized
{
    #region Fields
    ...
    public ProductAttributeFormatterCustomized(ICurrencyService currencyService,
    ...
    /// The task result contains the attributes
    /// </returns>
    public virtual async Task<string> FormatAttributesAsync(Product product, string attributesXml, int languageId)
    {
        var customer = await _workContext.GetCurrentCustomerAsync();
        var currentStore = await _storeContext.GetCurrentStoreAsync();

        return await FormatAttributesByLanguageAsync(product, attributesXml, customer, currentStore, languageId);
    }
        ...
        bool allowHyperlinks = true)
    {
        ...
        return await FormatAttributesByLanguageAsync(product, attributesXml, customer, store, currentLanguage.Id, separator, htmlEncode, renderPrices,
            renderProductAttributes, renderGiftCardAttributes, allowHyperlinks);
    }

    /// <summary>
    /// Formats attributes with a specified language
    /// </summary>
    /// <param name="product">Product</param>
    /// <param name="attributesXml">Attributes in XML format</param>
    /// <param name="customer">Customer</param>
    ...
    /// The task result contains the attributes
    /// </returns>
    private async Task<string> FormatAttributesByLanguageAsync(Product product, string attributesXml,
        Customer customer, Store store, int languageId, string separator = "<br />", bool htmlEncode = true, bool renderPrices = true,
        bool renderProductAttributes = true, bool renderGiftCardAttributes = true,
        bool allowHyperlinks = true)
    {
        ...
        {
            var productAttributes = await _productAttributeParser.ParseProductAttributeMappingsAsync(attributesXml);
            foreach (var attribute in productAttributes)
            {
                var productAttribute = await _productAttributeService.GetProductAttributeByIdAsync(attribute.ProductAttributeId);
                var attributeName = attribute.TextPrompt.IsNullOrWhiteSpace() ?
                    await _localizationService.GetLocalizedAsync(productAttribute, a => a.Name, languageId) :
                    await _localizationService.GetLocalizedAsync(attribute, x => x.TextPrompt, languageId);

                //var attributeName = await _localizationService.GetLocalizedAsync(productAttribute, a => a.Name, languageId);
                ...
                else
                {
                    ...
                    {
                        var formattedAttribute = $"{attributeName}: {await _localizationService.GetLocalizedAsync(attributeValue, a => a.Name, languageId)}";
