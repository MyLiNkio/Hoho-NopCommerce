...
namespace Nop.Plugin.Widgets.qBoSlider.Service
{
    ...
    public class WidgetZoneSlideService : IWidgetZoneSlideService
    {
        ...
        public virtual IPagedList<WidgetZoneSlide> GetWidgetZoneSlides(int? widgetZoneId = null, int? slideId = null, int pageIndex = 0, int pageSize = int.MaxValue,
            ScreenTargetFiltering screenTarget = ScreenTargetFiltering.All, int mainOrChildSlideId = 0)
        {
            ...
            if (screenTarget == ScreenTargetFiltering.Universal)
                query = query.Where(x => !x.UseOnMobile);
            else if (screenTarget == ScreenTargetFiltering.UseOnMobile)
                query = query.Where(x => x.UseOnMobile);

            if(mainOrChildSlideId > 0)
                query = query.Where(x => x.MainOrChildSlideId == mainOrChildSlideId);
        ...
        public virtual async Task InsertWidgetZoneSlideAsync(WidgetZoneSlide widgetZoneSlide)
        {
            ...
            if(widgetZoneSlide.UseOnMobile && widgetZoneSlide.MainOrChildSlideId > 0)
            {
                var mainSlide = await _widgetZoneSlideRepository.GetByIdAsync(widgetZoneSlide.MainOrChildSlideId);
                if(mainSlide != null)
                {
                    mainSlide.MainOrChildSlideId = widgetZoneSlide.Id;
                    _widgetZoneSlideRepository.Update(mainSlide);
                }
            }
        }
        ...
        public virtual async Task DeleteWidgetZoneSlideAsync(WidgetZoneSlide widgetZoneSlide)
        {
            ...
            {
                var mainSlide = await _widgetZoneSlideRepository.GetByIdAsync(widgetZoneSlide.MainOrChildSlideId);
                if(mainSlide != null && mainSlide.MainOrChildSlideId > 0)
                {
                    mainSlide.MainOrChildSlideId = 0;
                    await _widgetZoneSlideRepository.UpdateAsync(mainSlide);
                }
            }
            else if(!widgetZoneSlide.UseOnMobile && widgetZoneSlide.MainOrChildSlideId > 0)
            {
                var mobileSlide = await _widgetZoneSlideRepository.GetByIdAsync(widgetZoneSlide.MainOrChildSlideId);
                if (mobileSlide != null)
                {
                   await _widgetZoneSlideRepository.DeleteAsync(mobileSlide);
                }
            }
        }
