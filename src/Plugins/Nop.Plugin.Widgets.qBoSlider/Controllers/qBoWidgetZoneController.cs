...
namespace Nop.Plugin.Widgets.qBoSlider.Controllers
{
    ...
    public class qBoWidgetZoneController : BasePluginController
    {
        ...
        public virtual async Task<IActionResult> AddWidgetZoneSlidePopup(int widgetZoneId, int targetScreenVariant = (int)ScreenTargetFiltering.Universal, int mainOrChildSlideId = 0)
        {
            //return access denied result if customer has no permissions
            if (!await _permissionService.AuthorizeAsync(StandardPermission.Configuration.MANAGE_WIDGETS))
                return AccessDeniedView();
            ...
            var model = new AddWidgetZoneSlideModel()
            {
                ...
                MainOrChildSlideId = mainOrChildSlideId,
                ScreenTargetId = targetScreenVariant
            };
        ...
        public virtual async Task<IActionResult> AddWidgetZoneSlidePopup(AddWidgetZoneSlideModel model)
        {
            ...
            var displayOrder = 0;
            if (model.ScreenTargetId != (int)ScreenTargetFiltering.UseOnMobile)
            {
                displayOrder = widgetZoneSlides.Any() ? widgetZoneSlides.Max(x => x.DisplayOrder) : 0;
            }

            //if adding mobile targer slide version I take only first selected element, all other are ignored as one Universal slide may contain only one Mobile tagret slide version.
            if (model.ScreenTargetId == (int)ScreenTargetFiltering.UseOnMobile)
            {
                model.SelecetedSlideIds = new List<int>{ model.SelecetedSlideIds.First() };
            }

            foreach (var slideId in model.SelecetedSlideIds)
            {
                var slide = new WidgetZoneSlide()
                {
                    SlideId = slideId,
                ...
                if (model.ScreenTargetId == (int)ScreenTargetFiltering.UseOnMobile)
                {
                    slide.UseOnMobile = true;
                    slide.MainOrChildSlideId = model.MainOrChildSlideId;

                    

                }

                else if (model.ScreenTargetId == (int)ScreenTargetFiltering.Universal)
                {
                    slide.UseOnMobile = false;
                }
                
                await _widgetZoneSlideService.InsertWidgetZoneSlideAsync(slide);
            }
        ...
        //[HttpPost]
        //public virtual async Task<IActionResult> AddSlideList(int targetScreenVariant = (int)ScreenTargetFiltering.Universal)
        //{
        //    //return access denied result if customer has no permissions
        //    if (!await _permissionService.AuthorizeAsync(StandardPermission.Configuration.MANAGE_WIDGETS))
        //        return await AccessDeniedJsonAsync();

        //    var gridModel = await _widgetZoneSlideModelFactory.PrepareAddWidgetZoneSlideModelAsync(new AddWidgetZoneSlideModel { ScreenTargetId = targetScreenVariant});
