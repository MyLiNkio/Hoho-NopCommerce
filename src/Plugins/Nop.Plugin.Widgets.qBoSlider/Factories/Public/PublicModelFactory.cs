...
namespace Nop.Plugin.Widgets.qBoSlider.Factories.Public
{
    ...
    public class PublicModelFactory : IPublicModelFactory
    {
        ...
        protected virtual async Task<WidgetZoneModel.SlideModel> PrepareSlideModel(WidgetZoneSlide widgetZoneSlide, Slide slide, int languageId)
        {
            ...
            var slideModel = new WidgetZoneModel.SlideModel()
            {
                Id = slide.Id,
                ...
                Description = await _localizationService.GetLocalizedAsync(slide, z => z.Description, languageId),
            };
        ...
        protected virtual async Task<WidgetZoneModel> PrepareSliderModel(WidgetZone widgetZone, int languageId, int storeId)
        {
            ...
            var model = new WidgetZoneModel()
            {
                ...
                MobileScreensMaxValue = settings.MobileScreenMaxSize
            };

            //add slide models to widget zone slider
            var widgetZoneSlides = _widgetZoneSlideService.GetWidgetZoneSlides(widgetZone.Id).Where(x=>!x.UseOnMobile).OrderBy(s => s.DisplayOrder);
            foreach (var widgetSlide in widgetZoneSlides)
            {
                ...
                if (!widgetSlide.UseOnMobile && widgetSlide.MainOrChildSlideId > 0)
                {
                    var mobileWidgetZoneSlide = await _widgetZoneSlideService.GetWidgetZoneSlideAsync(widgetSlide.MainOrChildSlideId);

                    var mobileSlide = await _slideService.GetSlideByIdAsync(mobileWidgetZoneSlide != null ? mobileWidgetZoneSlide.SlideId : 0);

                    //don't display unpublished slides
                    if (mobileSlide != null && mobileSlide.Published && slide.PublishToday() && await _aclService.AuthorizeAsync(slide) && await _storeMappingService.AuthorizeAsync(slide, storeId))
                    {
                        slideModel.MobileScreenSlide = await PrepareSlideModel(mobileWidgetZoneSlide, mobileSlide, languageId);
                    }
                }
